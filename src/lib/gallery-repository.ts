...
export class GalleryRepository {
  ...
  private static async deleteCloudinaryImage(imageUrl: string): Promise<void> {
    ...
    try {
      ...
      if (publicId) {
        ...
        logger.info(`[GalleryRepository] Deleted Cloudinary image: ${publicId}`)
      }
    } catch (error) {
      logger.error('[GalleryRepository] Failed to delete Cloudinary image', { error })
  ...
  static async getAll(options?: {
    ...
    try {
      ...
      logger.info(`[GalleryRepository] Retrieved ${result.length} items from database`)
      return result as unknown as GalleryItem[]
    } catch (error) {
      logger.error('[GalleryRepository] Error fetching gallery items', { error })
  ...
  static async create(data: CreateGalleryItem): Promise<GalleryItem> {
    try {
      ...
      logger.info('[GalleryRepository] Creating gallery item', insertData)
      ...
      logger.info('[GalleryRepository] Gallery item created successfully', { id: newItem.id })
      return newItem as GalleryItem
    } catch (error) {
      logger.error('[GalleryRepository] Error creating gallery item', { error })
  ...
  static async update(id: string, data: UpdateGalleryItem): Promise<GalleryItem | null> {
    try {
      ...
      logger.info('[GalleryRepository] Updating gallery item', { id, updateData })
      ...
      logger.info('[GalleryRepository] Gallery item updated successfully', { id })
      return result[0] as unknown as GalleryItem
    } catch (error) {
      logger.error('[GalleryRepository] Error updating gallery item', { error })
  ...
  static async delete(id: string): Promise<boolean> {
    try {
      ...
      logger.info('[GalleryRepository] Deleting gallery item', { id })
      ...
      logger.info('[GalleryRepository] Gallery item deleted successfully', { id })
      return true
    } catch (error) {
      logger.error('[GalleryRepository] Error deleting gallery item', { error })
